        ...
        <div class="col-12">
          <div class="input-group d-flex justify-content-end mb-4">
            @if(\Auth::user()->hasRole('add-users'))
              <a class="btn btn-info mx-2" href="{{route('create.user')}}">
                ...
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$user->name}}</td>
                      <td>{{$user->email}}</td>
                      <td>{{\App\Models\Group::find($user->group_id)->name}}</td>
                      <td><span class="badge" style="background: {{ $user->is_enabled ? 'green' : 'red' }};">
                      ...
                      <td>
                        ...
                        <a href="{{route('edit.user',['user' => $user])}}" class="text-dark font-weight-bold text-xs" style="margin-right:0.5rem">
                            <i class="material-icons opacity-10">edit</i>
                        </a>
                        @endif
                        @if(\Auth::user()->hasRole('delete-users'))
                        <a href="{{route('delete.user',['user' => $user])}}" class="text-dark font-weight-bold text-xs">
