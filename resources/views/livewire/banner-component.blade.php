      ...
      <div class="row">
        <div class="col-12">
          <div class="input-group d-flex justify-content-end mb-4">
            @if(\Auth::user()->hasRole('add-banners'))
              <a class="btn btn-info mx-2" href="{{route('create.banner')}}">
              ...
              <div class="table-responsive">
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$banner->name}}</td>
                      <td><span class="badge" style="background: {{ $banner->is_enabled ? 'green' : 'red' }};">
                        {{$banner->is_enabled ? "Active" : "Inactive"}}
                      </span></td>
                      <td>{{$banner->created_at->format('Y-m-d')}}</td>
                      <td>
                        @if(\Auth::user()->hasRole('edit-banners'))
                        <a href="{{route('edit.banner',['banner' => $banner])}}" class="text-dark font-weight-bold text-xs" style="margin-right:0.5rem">
                            <i class="material-icons opacity-10">edit</i>
                        </a>
                        @endif
                        @if(\Auth::user()->hasRole('delete-banners'))
                        <a href="{{route('delete.banner',['banner' => $banner])}}" class="text-dark font-weight-bold text-xs">
